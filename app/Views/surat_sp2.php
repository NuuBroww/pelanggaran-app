...
<head>
    ...
    <title>SURAT PERINGATAN KEDUA - <?= esc($sp['nama_santri']) ?></title>
    <style>
        @page { 
            margin: 1cm;
            size: A4 portrait;
        ...
        /* TAMBAHKAN INI DI CSS YANG SUDAH ADA */
...
.indented-block {
    margin: 10px 0 15px 30px; /* INI YANG BIKIN INDENT - margin kiri 30px */
    padding: 0;
    text-align: justify;
}
...
<body>
    ...
    <div id="editForm" class="edit-form no-print" style="display: none;">
        <h3 style="margin-top: 0; font-size: 16px;">Edit Isi Surat Peringatan Kedua</h3>
           ...
           value="<?= formatTanggalIndonesia($sp['tanggal_surat']) ?>" readonly>
            ...
            <div class="form-group">
                ...
                <div class="format-info">
                    <strong>Format khusus SP2:</strong><br>
                    - Gunakan //bteks//b untuk <strong>tebal</strong><br>
                    - Untuk list, gunakan format: 1. point pertama, 2. point kedua, dst<br>
                    - Contoh:<br>
                    <em>1. Wajib membayar SPP Normal sebesar //bRp.1.750.000,-//b selama satu bulan<br>
                    2. Jika //bdalam kurun waktu 1//b semester kedepan ananda masih memiliki poin pelanggaran di atas 200 serta sikap dan perilaku nya belum berubah menjadi lebih baik, maka akan dikenakan sanksi berupa //bSurat Peringatan Ketiga//b</em>
                </div>
            </div>
    ...
    <div class="nomor-surat">
        <strong>SURAT PERINGATAN KEDUA</strong><br>
...
function formatSP2Content($text) {
    ...
    if (preg_match('/^\d+\./', $text)) {
        $lines = explode("\n", $text);
        $output = '<div class="formatted-list">';
        ...
        foreach ($lines as $line) {
            ...
            if (!empty($line)) {
                ...
                if (preg_match('/^(\d+)\.\s*(.*)/', $line, $matches)) {
                    ...
                    $output .= '<span class="list-number">' . $matches[1] . '.</span>';
                    $output .= $matches[2];
                    $output .= '</div>';
                } else {
                    $output .= '<div class="list-item">' . $line . '</div>';
                }
            }
        }
        $output .= '</div>';
